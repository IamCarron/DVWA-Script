...
if [ -d "/var/www/html/DVWA" ]; then
    ...
    read -p "$(get_language_message "en" "\e[96mDo you want to delete the existing folder and download it again (y/n):\e[0m " "es" "\e[96m¿Desea borrar la carpeta existente y descargarla de nuevo? (s/n):\e[0m ")" user_response
    ...
    if [[ "$user_response" == "s" || "$user_response" == "y" ]]; then
        ...
        download_message=$(get_language_message "en" "\e[96mDownloading DVWA from GitHub...\e[0m" "es" "\e[96mDescargando DVWA desde GitHub...\e[0m")
        echo -e "$download_message"
        git clone https://github.com/digininja/DVWA.git /var/www/html/DVWA
        sleep 2
    elif [ "$user_response" == "n" ]; then
        # El usuario elige no descargar / User chooses not to download
        no_download_message=$(get_language_message "en" "\e[96mContinuing without downloading DVWA.\e[0m" "es" "\e[96mContinuando sin descargar DVWA.\e[0m")
        echo -e "$no_download_message"
    else
        # Respuesta inválida / Invalid answer
        invalid_message=$(get_language_message "en" "\e[91mError! Invalid response. Exiting the script.\e[0m" "es" "\e[91m¡Error! Respuesta no válida. Saliendo del script.\e[0m")
